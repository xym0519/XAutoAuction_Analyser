...
function AtrSearch:AnalyzeResultsPage()
	...
	if (self.current_page == 1 and totalAuctions > 8000) then -- give Blizz servers a break
	...
	if (numBatchAuctions > 0) then
		...
		for x = 1, numBatchAuctions do
			...
			local xxKey = name .. ":" .. (time() - time() % 60);
			if (XXScanHistory[xxKey] == nil) then
			    local xxItem = {itemName= name, maxPrice= buyoutPrice/count, minPrice= buyoutPrice/count, sumPrice= buyoutPrice, count= count};
			    XXScanHistory[xxKey] = xxItem;
			else
			    local xxItem = XXScanHistory[xxKey];
			    if (xxItem.maxPrice < buyoutPrice/count) then
			        xxItem.maxPrice = buyoutPrice/count;
			    end
			    if (xxItem.minPrice > buyoutPrice/count) then
			        xxItem.minPrice = buyoutPrice/count;
			    end
			    xxItem.sumPrice = xxItem.sumPrice + buyoutPrice;
			    xxItem.count = xxItem.count + count;
			    XXScanHistory[xxKey] = xxItem;
			end
