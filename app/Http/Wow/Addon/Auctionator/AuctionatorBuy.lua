	...
	while (true) do
		...
		if (zc.StringSame (name, gAtr_Buy_ItemName) and buyoutPrice == gAtr_Buy_BuyoutPrice and count == gAtr_Buy_StackSize) then
			...
			if (andBuy and gAtr_Buy_NumUserWants > gAtr_Buy_NumBought) then
				...
				local buyTime = time();
                local xxKey = name .. ":" .. time() .. ":" .. XXBuyIndex;
                XXBuyIndex = XXBuyIndex + 1;
                local buyHistory = {itemName= name, buyTime= buyTime, price= buyoutPrice/count, count=count};
                XXBuyHistory[xxKey] = buyHistory;
